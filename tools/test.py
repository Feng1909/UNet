...
class Tester:
    ...
    def find_threshold(self):
        ...
        for val_image in ann['val']:
            image_ori = cv2.imread(f'{self.cfgs.dataloader.dataset.data_folder}/train/im/{val_image}')[:,:,0] / 255.
            image_name_label = val_image[:-4]+'.png'
            target = cv2.imread(f'{self.cfgs.dataloader.dataset.data_folder}/train/gt/{image_name_label}')[:,:,0] / 255.
        ...
        for threshold in tqdm(thresholds):
            ...
            # print(type(preds_.reshape(-1)), targets.reshape(-1).tolist())
            f1s.append(f1_score(preds_.reshape(-1).astype(np.int64), targets.reshape(-1).astype(np.int64)))
        f1s = np.stack(f1s)
        threshold = thresholds[f1s.argmax()]
        print(f'best f1 score is {f1s.max()} at threshold = {threshold}')

        # write valid results
        for val_image, pred, target, ori in zip(val_images, preds, targets, oris):
            pred[pred >= threshold] = 1
            pred[pred < threshold] = 0

            cat = np.concatenate([ori, pred, target], axis=1)*255
