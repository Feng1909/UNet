...
class Normalize(object):
    def __call__(self, sample):
        ...
        label_128 = cv2.resize(label, (256, 256), interpolation=cv2.INTER_AREA)
        label_64 = cv2.resize(label, (128, 128), interpolation=cv2.INTER_AREA)
        label_32 = cv2.resize(label, (64, 64), interpolation=cv2.INTER_AREA)
...
class Mosaic(object):
    ...
    def __call__(self, sample):
        ...
        if random.random() <= self.p:
            ...
            img2 = cv2.imread(f'{self.data_folder}/train/im/{image_names[0]}')[:,:,0]
            img3 = cv2.imread(f'{self.data_folder}/train/im/{image_names[1]}')[:,:,0]
            img4 = cv2.imread(f'{self.data_folder}/train/im/{image_names[2]}')[:,:,0]
